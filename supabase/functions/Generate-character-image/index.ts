...
Region references are authoritative only for landscape, architecture, vegetation, climate, atmosphere and environmental regional identity. When supplied and JSON gives no extra environmental facts, do NOT invent environmental specifics. Instead, explicitly instruct FLUX to make the supplied canonical region reference clearly recognizable in the background: faithfully preserve its major landscape structure, atmosphere and distinctive visible environmental features while keeping the character dominant. The environment must occupy enough of the composition to establish the location clearly, without competing with or obscuring the character. Aim for a character-first visual balance of roughly 70% character emphasis and 30% environmental presence; this is a composition guideline, not a literal pixel measurement. Use wording such as "behind the character, a clearly visible environment faithfully follows the supplied canonical region reference, preserving its major landscape structure, atmosphere and distinctive environmental features while remaining secondary to the character." A region reference must never alter character race, anatomy, face, morphology, body type, species markers, equipment, clothing or powers.
...
FINAL AUDIT
Before returning, internally verify: every CRITICAL fact and visible_expression appears concretely in flux_prompt; no CRITICAL item exists only as an abstract label; for every non-human character, mentally remove powers, magical effects, clothing and equipment and verify that the body itself would still clearly read as the required race/species according to the canonical race reference—if not, strengthen intrinsic racial anatomy in flux_prompt; for every required physical/identical clone, verify that its race, face, age, morphology, proportions and scale match the original unless the JSON explicitly says otherwise; morphology/age/scale/race/anatomy/colors/equipment remain faithful; nothing was normalized or beautified; no unsupported racial/cultural/regional/factional/technological lore was invented; unsupported region/race specifics are left to canonical references; when a canonical region reference is supplied, flux_prompt explicitly requires a clearly visible and recognizable background faithful to that reference rather than a generic backdrop; no visible consequence was discarded because its parent mechanic is abstract; intensity/mastery/severity guides relevant effects without displaying numbers; multiple effects remain distinguishable; required secondary beings are represented; relative scale is faithful; environment stays secondary but meaningfully present; names/titles/IDs are omitted; prose is coherent; no written text is requested. If any check fails, revise before answering.
