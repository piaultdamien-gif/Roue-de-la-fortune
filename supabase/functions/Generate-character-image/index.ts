...
Region references are authoritative only as a VISUAL VOCABULARY for landscape, architecture, vegetation, materials, climate, lighting and atmosphere. They are NEVER composition templates. Extract the region's recognizable environmental language, then invent a NEW location and NEW arrangement that clearly belongs to the same region. NEVER copy or closely reproduce the reference's camera angle, horizon, perspective, landmark placement, silhouette layout, foreground/background arrangement, paths, rivers, cliffs, buildings, vegetation placement, character placement, pose, or overall composition. Do not ask FLUX to preserve the reference's "major landscape structure". Similarity should come from regional identity, not scene duplication. The environment must remain clearly readable but secondary to the protagonist, with roughly 70% character emphasis and 30% environmental presence. A region reference must never alter character race, anatomy, face, morphology, body type, species markers, equipment, clothing or powers.
...
REFERENCE USAGE — IDENTITY WITHOUT COPYING:
The supplied reference images are canonical visual dictionaries, NOT composition templates.
Create an original scene from scratch.
Do not reproduce or closely imitate any reference image's camera angle, framing, perspective, pose, character placement, horizon, landmark placement, foreground/background layout, silhouette arrangement, or overall scene composition.
Race references may guide intrinsic racial anatomy and biological identity only.
The region reference may guide environmental vocabulary only: characteristic terrain, materials, architecture, vegetation, climate, lighting and atmosphere. Use those ingredients to invent a DIFFERENT location and DIFFERENT arrangement within the same region.
The final image must be recognizably consistent with the references while clearly being a new composition.`;
    }
...
Reference images 0-3 are canonical character/region references selected for this character. Race references govern racial anatomy only; the region reference supplies environmental vocabulary only. Do not reproduce the camera angle, framing, perspective, pose, character placement, horizon, landmark placement, foreground/background layout, or overall composition of any reference. Invent a clearly different scene within the same canonical region while preserving its recognizable environmental identity. Preserve the generated character data and do not copy unrelated clothing, pose, weapon, or identity from references.`;
