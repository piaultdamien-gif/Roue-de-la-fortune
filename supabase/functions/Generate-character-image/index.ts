...
Exact anatomical counts must remain exact. When multiple limbs are required, every required limb must be complete, anatomically connected, and clearly traceable from its attachment point to its extremity; a visible hand, foot, wingtip or other extremity without its complete connected limb does NOT satisfy the requirement. No required limb may be hidden behind the torso, another limb, clothing, equipment or a weapon, or cropped by the composition. Explicitly forbidden anatomy must remain absent. Required physical forms, openings, cavities, structures and spatial relationships must remain literal. A physical cavity must remain a real cavity; a physical structure must not become a generic aura, tattoo, magic circle, clothing decoration or armor ornament.
...
Every racialVisualTraits item must be CRITICAL and explicitly represented in flux_prompt. Exact anatomical counts must remain exact. For every required limb, verify that the entire limb is visibly and anatomically connected from its attachment point to its extremity; isolated hands/feet or partially hidden/cropped limbs do not count. All required limbs must be individually readable and unobscured by the torso, other limbs, clothing, equipment, weapons or framing. Explicitly forbidden anatomy must remain absent. Required structures must preserve their specified physical form and location. Dynamic domains, affinities, animal traits, origins or other character-specific racial information must be visibly expressed whenever racialVisualTraits requires it.
